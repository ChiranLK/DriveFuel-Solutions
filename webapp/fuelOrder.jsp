...
<%
    ...
    String fuelname = request.getSession().getAttribute("fuelname").toString();
    Fuel fuel = FuelServlet.getFuel(fuelname);
...
<head> 
    ...
    <script type="text/javascript">
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const pricePerLiter = <%= fuel.getPrice() %>;
...
<div class="container">
    <div class="divbox">
        ...
        <p>You are purchasing: <strong><%= fuel.getName() %></strong></p>
        <p>Price Per 1 Liter: Rs.<strong><%= fuel.getPrice() %></strong></p>

        <form action="OrderServlet" method="post">
            <input type="hidden" name="status" value="pending">
            <input type="hidden" name="item" value="<%= fuel.getName() %>">
            <input type="hidden" name="redirect" value="<%= request.getRequestURI() %>">
            <input type="hidden" name="action" value="create">
            <input type="hidden" name="email" value="<%= userx.getEmail() %>">
            <input type="hidden" name="unitprice" value="<%= fuel.getPrice() %>">
            <label for="quantity">Quantity (Liters):</label>
            <input type="number" id="quantity" name="quantity" value="1" min="1" max="100" required>
            <input type="hidden" id="type" name="type" value="Fuel">
    ...
    <div class="divbox">
        ...
        </table><!-- navigate to checkout.jsp -->
